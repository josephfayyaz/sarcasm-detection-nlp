...
def train_binary_model(
    ...
    def _maybe_update_best(
            ...
            """
            Update best weights IN MEMORY (CPU) only when improved.
            This avoids disk IO inside the training loop (faster).
            """
            if current > (best + float(min_delta)):
                # store on CPU to avoid GPU memory blow
    ...
    for lr in learning_rates:
        ...
        if monitor == "f1_macro":
            f1_macro = float(best_metric_this_lr)
        else:
