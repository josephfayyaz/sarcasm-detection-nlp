...
def main():
    ...
    parser.add_argument("--models_root",  help="Folder containing model subfolders", default=r".\src\dataset_translated\model_output")
    parser.add_argument("--validation_csv",  help="CSV with columns: text, label, variety (and optionally source)", default=r".\src\dataset_translated\dataset\valid-new.csv")
    parser.add_argument("--output_prefix", help="Prefix for output CSV files",default="tdata_")
    ...
    out_variety.to_csv(fr".\src\dataset_translated\results\{args.output_prefix}_per_variety.csv", index=False)
    print(f"\nSaved: {args.output_prefix}_per_variety.csv")

    if len(all_source) > 0:
        out_source = pd.DataFrame(all_source).sort_values(["model", "source"], ascending=[True, True])
        out_source.to_csv(fr".\src\dataset_translated\results\{args.output_prefix}_per_source.csv", index=False)
    ...
    if len(all_variety_source) > 0:
        ...
        out_vs.to_csv(fr".\src\dataset_translated\results\{args.output_prefix}_per_variety_source.csv", index=False)
