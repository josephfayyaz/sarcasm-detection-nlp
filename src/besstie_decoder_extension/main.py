...
# Supports training + inference, and now supports custom decoder heads.
...
def allowed_keys() -> Dict[str, Set[str]]:
    return {
        ...
        "train": {
            ...
            "fp16", "bf16", "tf32",
            # ---- Extension: custom decoder heads ----
            "decoder_type",           # hf_default | cnn | attn_pool
            "decoder_dropout",        # float
            "cnn_num_filters",        # int
            "cnn_kernel_sizes",       # list[int]
            "attn_mlp_hidden",        # int | null
        },
        "predict": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    t.add_argument("--decoder_type", type=str, default=None, choices=["hf_default", "cnn", "attn_pool"])
...
def main():
    ...
    if args.command == "train":
        # Required config values (existing ones)
        ...
        # Optional extension config values (defaults keep old behavior)
        ...
        # attn_mlp_hidden: only override if user provides it (None is meaningful)
