...
(AutoModelForSequenceClassification, e.g., RoBERTa-large fine-tuned).

What it does:
- Loads your fine-tuned classifier from checkpoint_dir (works with .safetensors)
- Applies several "prompt templates" to the same text (direct, definition, context, evidence)
- Gets probabilities from the SAME trained model for each template
- Aggregates predictions (majority vote / mean prob / weighted mean prob)
- Evaluates macro-F1 overall and per variety (en-AU, en-IN, en-UK)
- Writes a CSV with per-template probs and final prediction

This is the correct way to implement "multi-prompt inference" WITHOUT switching to Llama/Qwen/etc.
"""
...
# ----------------------------
# Templates = "multi-prompt" for a classifier
# ----------------------------
...
# ----------------------------
# Metrics (macro F1 like the paper)
# ----------------------------
...
def predict_probs_for_texts(
    ...
    """
    Returns: probs array shape (N, 2) for labels [0,1]
    """
    ...
    iterator = loader
    if tqdm is not None:
        iterator = tqdm(loader, desc="Inference", leave=False)
...
def aggregate_probs(
    ...
    if method == "majority":
        # vote based on argmax for each template
    ...
    if method == "mean":
        avg = np.mean(np.stack(per_template_probs, axis=0), axis=0)  # (N,2)
...
def run_multi_prompt_classifier(
    ...
    """
    Loads model+tokenizer from checkpoint_dir and runs multi-template inference on df.
    """
    # Filter to sarcasm task rows (BESSTIE contains Sentiment and Sarcasm)
    if "task" in df.columns:
        df = df[df["task"].astype(str) == task_filter].copy()

    if df.empty:
        raise ValueError(f"No rows found after filtering task='{task_filter}'. Check your CSV 'task' column.")
...
def evaluate(df: pd.DataFrame, label_col: str = "label", group_col: Optional[str] = None) -> None:
    if label_col not in df.columns:
        print(f"[warn] No ground-truth column '{label_col}' found. Skipping evaluation.")
        return
    ...
    print(f"F1 (macro):  {f1_mac:.4f}   <-- paper-style")
...
def main():
    ...
    parser.add_argument("--task_filter", type=str, default="Sarcasm",
                        help="Filter rows by df['task'] (default: Sarcasm).")
    parser.add_argument("--label_col", type=str, default="label")
    parser.add_argument("--group_col", type=str, default="variety",
                        help="Group-wise evaluation column (default: variety).")
    ...
    # You can tune weights here (this is part of the novelty study)
    ...
    print("Columns include per-template p0/p1 and final_pred/final probabilities.")
